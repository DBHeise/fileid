...
namespace oless {
	namespace excel {
		...
		public:
			...
			void ParseStream(std::vector<uint8_t> stream) {
				...
				while (this->index + 4 < max)
				{
					unsigned short recordType = common::ReadUShort(buffer, max, this->index);
					this->index += 2;
					unsigned short recordSize = common::ReadUShort(buffer, max, this->index);
					this->index += 2;

					std::vector<uint8_t> data;
					if (this->index + recordSize < max) {
					...
					if (r != nullptr) {
						...
						case 0x002F: //FilePassRecord
							this->index += max; //exit, we're not decrypting, so we won't be able to parse the rest correctly
							break;
						default:
							break;
						}
