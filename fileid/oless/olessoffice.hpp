...
namespace oless {
	...
	public:
		...
		//Word (.doc) specific
		unsigned short GetnFib(POLE::Stream* stream) {
			unsigned short nFib = 0;


			Word::Fib* fib = OleHelper::GetStructFromStream<Word::Fib>(stream);

			nFib = fib->base.nFib;
			//if (fib->cbRgFcLcb > 0x00FF) {		//Corrupted file	}	

			stream->seek(sizeof(Word::Fib) + (fib->cbRgFcLcb * 8));

			Word::Fib2* fib2 = OleHelper::GetStructFromStream<Word::Fib2>(stream);
			if (fib2->cswNew != 0) {
				nFib = fib2->nFibNew;
			}
		...
		unsigned short GetPptVersion(POLE::Storage* storage, POLE::Stream* stream) {
			...
			PowerPoint::UserEditAtom* uea = OleHelper::GetStructFromStream<PowerPoint::UserEditAtom>(stream);
			version = uea->version;
		...
		unsigned short GetPubVersion(POLE::Stream* stream) {
			unsigned short version = 0;

			Publisher::PubHeader* header = OleHelper::GetStructFromStream<Publisher::PubHeader>(stream);
		...
		//Visio (.vsd) specific
		unsigned short GetVisoVersion(POLE::Stream* stream) {
			unsigned short version = 0;

			Visio::VsdHeader* header = OleHelper::GetStructFromStream<Visio::VsdHeader>(stream);
